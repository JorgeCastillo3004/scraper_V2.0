...
| [sesion_2026-09-06_estado_y_siguiente.md](sesion_2026-09-06_estado_y_siguiente.md) | **EMPEZAR AQUÍ la próxima sesión** — qué quedó funcionando (live con Restart+Linger verificado, memoria arreglada, 875→15 pendientes, respaldo SofaScore al 97%), qué está bloqueado por decisión (scraper_v3, P7, core-db, permisos GitLab), siguiente trabajo por valor, comandos y los hallazgos que no hay que olvidar (hora placeholder, heartbeat que no late, nombres de liga que mienten) |
| [failover_respaldo.md](failover_respaldo.md) | **SC10+SC11: cuándo entra el respaldo** — 3 señales (latido por mtime del log, no por el heartbeat JSON que estaba roto; partidos colgados; atraso frente a SofaScore), umbrales calibrados con el ciclo real (134s+60s), e histéresis 3 lecturas para conmutar / 5 para volver. **El mando se decide por VITALIDAD, no por el veredicto global**: con los colgados el respaldo no devolvía nunca el mando. Simulador end-to-end sin escribir en la BD |
| [sofascore_mapeo.md](sofascore_mapeo.md) | **Cómo funciona el mapeo del respaldo** — los 3 archivos (`sofascore_overrides.json` manda, luego `sofascore_map.json` de ligas y `sofascore_teams_map.json` de equipos), cómo se generan, cómo elegir torneo (verificación **por equipos**, no por nombre) y cómo localizar el partido (fecha manda; la hora solo si es real). Estado: 12 ligas, 202 equipos, **139/144 partidos (97%)** en 5 deportes. La BD no se toca |
