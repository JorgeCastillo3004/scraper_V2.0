...
DASH_DEV_SKIP_AUTH = True  # LOGIN DESACTIVADO — activar en producción
SPORT_ICONS      = {
...
# Lock global para page.update() desde hilos — Flet 0.82.2 no es thread-safe
_GLOBAL_PAGE_LOCK = threading.Lock()


def safe_update(page: ft.Page):
    """Wrapper thread-safe para page.update(). Usar en todos los hilos daemon."""
...
def build_noticias_tab(page: ft.Page) -> ft.Control:
    ...
    dd_freq = ft.Dropdown(
        label='Frecuencia', width=160, value='0',
        options=[ft.dropdown.Option(key=v, text=lbl) for lbl, v in FREQ_OPTIONS],
    )
    tf_hora = ft.TextField(
        label='Hora inicio (HH:MM)', width=140, value='08:00',
        hint_text='08:00', visible=False,
    )
    lbl_proxima = ft.Text('', size=11, color=ft.Colors.GREY_500)

    _next_run = [None]  # datetime próxima ejecución

    def _update_hora_visibility(e=None):
        tf_hora.visible = dd_freq.value == '1440'
        try:
            page.update()
        except Exception:
            pass

    dd_freq.on_change = _update_hora_visibility

    def _compute_next() -> datetime | None:
        freq_min = int(dd_freq.value or '0')
        if freq_min == 0:
            return None
        now = datetime.now()
        if freq_min == 1440:
            try:
                h, m = map(int, tf_hora.value.strip().split(':'))
                ...
                pass
        return now + timedelta(minutes=freq_min)

    def _schedule_apply(e=None):
        nxt = _compute_next()
        _next_run[0] = nxt
        lbl_proxima.value = (f'Próxima ejecución: {nxt.strftime("%d/%m %H:%M:%S")}'
                             if nxt else '')
        try:
            page.update()
        except Exception:
    ...
    def _scheduler_loop():
        while True:
            time.sleep(30)
            nxt = _next_run[0]
            if nxt and datetime.now() >= nxt and not PM.is_running('noticias'):
                _do_start()
                _next_run[0] = _compute_next()
    ...
    def on_done(returncode, p):
        ...
        # Programar siguiente si hay frecuencia activa
        nxt = _compute_next()
        _next_run[0] = nxt
        lbl_proxima.value = (f'Próxima ejecución: {nxt.strftime("%d/%m %H:%M:%S")}' if nxt else '')
        try:
            p.update()
        except Exception:
            pass
    ...
    def _do_start():
        news_script = os.path.join(BASE_DIR, 'scripts', 'run_news.py')
        proc = PM.start('noticias', [sys.executable, news_script])
        if proc is None:
    ...
    return ft.Container(
        content=ft.Column([
            ...
            ft.Container(
                content=ft.Column([
                    ft.Text('Programación de ejecución:', size=12, weight=ft.FontWeight.BOLD),
                    ft.Row([
                        dd_freq, tf_hora,
                        ft.IconButton(
                            ft.Icons.SCHEDULE_ROUNDED,
                            on_click=_schedule_apply,
                            tooltip='Aplicar programación',
                            icon_size=18,
                        ),
                    ], spacing=12, vertical_alignment=ft.CrossAxisAlignment.CENTER),
                    lbl_proxima,
                ]),
                bgcolor=ft.Colors.with_opacity(0.06, ft.Colors.SECONDARY),
                padding=12, border_radius=8,
            ),
            ft.Divider(height=6),
            # ── Controles ─────────────────────────────────────────────────
...
def build_jugadores_tab(page: ft.Page) -> ft.Control:
    ...
    def load_player_stats():
        time.sleep(2)  # esperar inicialización completa de la página
        try:
            conn = psycopg2.connect(
                host=DB_HOST, dbname=DB_NAME, user=DB_USER, password=DB_PASS,
                connect_timeout=5, options="-c statement_timeout=5000",
...
def build_envivo_tab(page: ft.Page) -> ft.Control:
    ...
    def _tmux_ensure_session():
        ...
        if not _tmux_session_exists():
            ...
            time.sleep(0.5)
            _tmux_run(['send-keys', '-t', TMUX_SESSION, f'cd {BASE_DIR}', 'Enter'])
            time.sleep(0.3)
    ...
    sw_test_mode = ft.Switch(label='Modo prueba', value=False,
    ...
    def _tail_log():
        ...
        # Capturar estado actual de tmux al abrir el tab
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                log_append('[INFO] ── Contenido actual sesión tmux ──', page, flush=False)
                for captured_line in result.stdout.splitlines():
                    clean = _ANSI_RE.sub('', captured_line).strip()
                    if clean:
                        log_append(clean, page, flush=False)
                log_append('[INFO] ── Fin captura inicial — siguiendo output en tiempo real ──', page, flush=False)
                _safe_update()
        except Exception:
            pass
    ...
    def _init_pipe():
        ...
        if _tmux_session_exists():
            _tmux_run(['pipe-pane', '-t', TMUX_SESSION])
            time.sleep(0.1)
            _tmux_run(['pipe-pane', '-t', TMUX_SESSION, f'cat >> {LIVE_LOG_FILE}'])
    ...
    def on_toggle(e):
        ...
        if s == 'stopped':
            ...
            lbl_status.value = f'{"[TEST] " if test else ""}Estado: iniciando — {", ".join(selected)}  |  intervalo: {interval_val}s'
            lbl_status.color = ft.Colors.ORANGE_300 if test else ft.Colors.RED_400
            _apply_toggle_look()
            log_append(f'{"[TEST] " if test else "[INFO] "}Iniciando — deportes: {", ".join(selected)}  intervalo: {interval_val}s', page, flush=False)
            _safe_update()
            ...
            def _start_bg():
                ...
                if test:
                    log_append('[TEST] write_control("none")', page, flush=False)
                    log_append('[TEST] tmux_ensure_session()', page, flush=False)
                    log_append(f'[TEST] tmux_send: {cmd}', page, flush=False)
                    _safe_update()
                    return
            ...
            _set_state('paused')
            lbl_status.value = f'{"[TEST] " if test else ""}Estado: pausando...'
            lbl_status.color = ft.Colors.ORANGE_300
            _apply_toggle_look()
            log_append(f'{"[TEST] " if test else "[INFO] "}Comando pause enviado', page, flush=False)
            _safe_update()
            if test:
                threading.Thread(target=lambda: (log_append('[TEST] write_control("pause")', page), _safe_update()), daemon=True).start()
            else:
                threading.Thread(target=_write_live_control, args=('pause',), daemon=True).start()

        elif s == 'paused':
            _set_state('running')
            lbl_status.value = f'{"[TEST] " if test else ""}Estado: reanudando...'
            lbl_status.color = ft.Colors.ORANGE_300 if test else ft.Colors.RED_400
            _apply_toggle_look()
            log_append(f'{"[TEST] " if test else "[INFO] "}Comando resume enviado', page, flush=False)
            _safe_update()
            if test:
                threading.Thread(target=lambda: (log_append('[TEST] write_control("resume")', page), _safe_update()), daemon=True).start()
            else:
    ...
    def on_stop(e):
        ...
        spinner.visible  = False
        lbl_status.value = f'{"[TEST] " if test else ""}Estado: deteniendo...'
        lbl_status.color = ft.Colors.ORANGE_400
        _apply_toggle_look()
        log_append(f'{"[TEST] " if test else "[INFO] "}Comando stop enviado', page, flush=False)
        _safe_update()

        def _stop_bg():
            if test:
                log_append('[TEST] write_control("stop")', page, flush=False)
                log_append('[TEST] esperar 4s + tmux_interrupt()', page, flush=False)
                _safe_update()
                # Simular el estado final después del stop
                time.sleep(1)
                _set_state('stopped')
                _apply_toggle_look()
                lbl_status.value = '[TEST] Estado: detenido'
                lbl_status.color = ft.Colors.GREY_400
                _safe_update()
                return
    ...
    return ft.Container(
        content=ft.Column([
            ...
            ft.Divider(height=8),
            # ── Controles ─────────────────────────────────────────────
            ft.Row([btn_toggle, btn_stop, btn_ver_tmux, sw_test_mode], spacing=8,
                   vertical_alignment=ft.CrossAxisAlignment.CENTER),
